...
1. Now that we've had a quick view of the dataset, let's get more details about its structure. Sometimes viewing a summary of the data structure is more useful than viewing the raw data, especially if the dataset has a large number of observations and/or rows. Run the code below to use the `glimpse` function to see a summary of the `trails` dataset.
    ...
    There are 739 observations in the trails dataset and 12 variables.
...
2. Before conducting statistical inference (or eventually fitting regression models),  we need do some exploratory data analysis (EDA). Much of EDA consists of visualizing the data but it also includes calculating summary statistics for the variables in our dataset. Let's begin by examining the distribution of `status` with a data visualization and summary statistics. 
    ...
    A bar graph is an appropriate visualization of the 'status' data because trails are either 'Proposed' or 'Existing', and the bar graph would show the proportion of each type.
    
    - Then, calculate the proportion of observations in each category of `status` by completing the code below.
    
    66.7% of the observations are existing trails and 33.3% are proposed trails.
...
6. Describe the distribution of `length`. Your description should include comments about the shape, center, spread, and any potential outliers. Use the graph from Exercise 4 and relevant summary statistics from Exercise 5 in your description.
  ...
  The distribution of length is skewed to the right with most trails being under 1 mile. The standard deviation of the trails is .852 miles because of the outlier trails, while the IQR is 0.727, which means that most trails lie between 0.2 and 0.95 miles. 
...
8. Consider the distribution of `class`. 
    ...
    This is the line of code I ran to see the counts of each type of class:
    
    table(current_trails$class, useNA = 'always')
    
    According to the table, there are 407 class 1 trails, 80 class 2 trails,
    2 class 3 trails, and 1 trail with no entry in the class field.
    ...
    A missing value of class can either be an input error or that a class has not
    yet been assigned to that trail.
...
```{r eval=FALSE}
current_trails <- current_trails %>% filter(class !=3) %>% 
  mutate(class = if_else(is.na(class), 1 ,class)) 

```
Since there are far more class 1 trails than any other kind, I assumed the N/A trail
was a class 1 trail.
...
10. Now that we've completed the univariate EDA (i.e. examining one variable at a time), let's examine the relationship between the length of the trail and its class variable. Make a graph to visualize the relationship between `length` and `class` and calculate the appropriate summary statistics. Include informative axis labels and title on your graph.
  ...
  I generated the graph using this code:
  
  ```
  current_trails %>% ggplot(aes(x=factor(class), y=length)) +
    stat_summary(fun=median, geom="bar") + 
    labs(title="Median Length of Trails by Class", x="Class", y="Length")

  ```
  
  I generated the summary statistics of each class with this code:
  
  ```
  class_1_trails <- current_trails %>%
    filter(class == 1)
    
  summary(class_1_trails$length)
  ```
  
  For trail 1, these are the summary statistics:
  Min: 0.01485, 1st Quartile: 0.19476, Median: 0.40833, Mean: 0.68193, 
  3rd Quartile: 0.82290, Max: 4.13138
  
  For trail 2, these are the summary statistics:
  Min: 0.04091, 1st Quartile: 0.30246, Median: 0.54844, Mean: 0.73338, 
  3rd Quartile: 1.03346, Max: 2.52198
...
11. Describe the relationship between `length` and `class`. In other words, describe how the distribution of `length` compares between trails that have different classes (1 = shared use bicycle and pedestrian path, 2 = bike lane, and 3 = bike route). Include information from the graph and summary statistics from the previous exercise in your response.
  ...
  Through the data, I can see that the median length of class 1 is greater than the median length of class 2, however class 1 does have an outlier. The median length for class 1 trails is 0.40833 miles while the median length for class 2 trails is 0.7338 miles. The spread of the data for class 2 is higher than class 1, given that the IQR for class 1 is 0.62814 and the IQR for class 2 is 0.731. 
...
12. The following conditions must be met when we conduct statistical inference on the difference in means between two groups. For each condition, specify whether it is met and a brief explanation of your reasoning. 
    - **Independence**
      The trails are independent since they are in different locations, we can assume
      that the length of one trail does not affect the length of another.
    - **Sample Size**
      The sample size for class 1 is 407 and the sample size for class 2 is 80.If we make the assumption that there are less than 4,070 class 1 trails and 800 class 2 trails, then the sample size of the trails data is large enough for statistical inference.
    - **Independent Groups**
      Since the trails are either in class 1 or class 2, we know that the groups are independent and there are no overlapping trails.
...
The null hypothesis is that there is no difference in the length of trails in class 1 and class 2.

The alternative hypothesis is that there is a difference in the lenght of trails in class 1 and class 2.
...
t.test(length ~ class, data = current_trails, 
...
15. Use the output from the previous exercise to answer the following: 
    ...
    The test statistic in this case is the difference between the mean length assuming that there is no difference between the length of trails. 
    
    - Write the definition of the p-value in the context of this analysis. 
    
    The p-value is the likelihood that there is no difference between class1 and class2 trails given the sample data.
    
    - State your conclusion in the context of this analysis. Use a significance level of $\alpha = 0.01$. 
    
    Since the p-value is 0.4852 (higher than 0.01), we can say the probability that the true difference in means is not equal to 0 is unlikely. 
...
16. Notice the confidence interval for the difference in mean trail length printed in the output from Exercise 14. Interpret this confidence interval in the context of this analysis. 
  ...
  The confidence interval given shows that we are 99% confident that the differences between the means of class 1 and class 2 lie between -0.243 adn 0.140, which again shows how the difference is likely to be 0. 
